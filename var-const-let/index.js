...
// NO es inmutable
    ...
    if(pokemonType === 'electric'){
        ...
        // cuando creamos un objeto constante podemos cambiar el valor de los keywords
        const pokemon = {
            name: 'Pikachu',
            type: 'eléctrico'
        }
        pokemon.name = 'Raichu' //esto si lo puedo hacer
        //pokemon = {} //esto no lo puedo hacer

        console.log(`${pokemon.name} de tipo ${pokemon.type}`);
    }
    if(true){
        let pokemonType = 'fuego'
        let pokemonName = 'Charizard'
        console.log(`${pokemonName} de tipo ${pokemonType}`);
    }
...
class Pokemon extends API{
    ...
    renderType(){
        const myFavoriteType = 'fire'
        let isMyFavoriteType = this.pokemon.types[0].type.name === 'fire'
        if(isMyFavoriteType){
            container.textContent = `Es de tipo ${this.pokemon.types[0].type.name}, mi tipo favorito 🤩`   
        }else{
